...
public class WakeNode {
    ...
    public AxisAlignedBB toBox() {
        return new AxisAlignedBB(this.x, this.y, this.z, this.x + 1, this.y + (1 - WakeNode.WATER_OFFSET), this.z + 1);
    }
    ...
    public static class Factory {
        public static Set<WakeNode> splashNodes(net.minecraft.entity.Entity entity, int y) {
            int res = WakeHandler.resolution.res;
            int w = (int) (0.8 * entity.width * res / 2);
            int x = (int) (entity.posX * res);
            int z = (int) (entity.posZ * res);
            ...
            return pixelsToNodes(pixelsAffected, y, WakesConfig.splashStrength, Math.abs(entity.motionY));
        }

        public static Set<WakeNode> rowingNodes(EntityBoat boat, int y) {
            Set<WakeNode> nodesAffected = new HashSet<>();
            double velocity = Math.sqrt(boat.motionX * boat.motionX + boat.motionZ * boat.motionZ);
            for (int i = 0; i < 2; i++) {
                if (boat.getPaddleState(i)) {
                    ...
                    if (0.3926991f / 2 <= phase && phase <= 0.7853982f + 0.3926991f) {
                        ...
                        double yawRad = Math.toRadians(boat.rotationYaw);
                        Vec3d dir = new Vec3d(-Math.sin(yawRad), 0, Math.cos(yawRad)).scale(velocity);
                        Vec3d from = paddlePos;
                        Vec3d to = paddlePos.add(dir.scale(2));
        ...
        public static Set<WakeNode> nodeLine(double x, int y, double z, float waveStrength, Vec3d velocity, float width) {
            ...
            return pixelsToNodes(pixelsAffected, y, waveStrength, hVel);
        }
